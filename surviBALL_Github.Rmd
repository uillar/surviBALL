...
surviBALL = coxph(Surv(eventTime, EFS) ~ lnckb.32044 + lnckb.61426 + lnckb.66409 + lnckb.83321 + lnckb.8576, data = norm_C1_CoxUV, x = T)
summary(surviBALL)
round(cox.zph(surviBALL)$table, 2)
ggforest(surviBALL)

# Aproximación de R²
concordancia <- survConcordance(Surv(eventTime, EFS) ~ predict(surviBALL))$concordance
r2_aprox <- 2 * (concordancia - 0.5)
r2_aprox
...
# predRisk alone
cox_covar = coxph(Surv(eventTime, EFSb) ~ predGr, data = norm_CoxUV, x = T)
summary(cox_covar)
...
# subtype + predGr
table(norm_CoxUV$predictedSubtype)
norm_CoxUV$Subtype_2 <- as.factor(norm_CoxUV$predictedSubtype)
levels(norm_CoxUV$Subtype_2)[levels(norm_CoxUV$Subtype_2) == "IKZF1_N159Y"] <- "Others"
levels(norm_CoxUV$Subtype_2)[levels(norm_CoxUV$Subtype_2) == "PAX5_P80R"] <- "Others"
levels(norm_CoxUV$Subtype_2)[levels(norm_CoxUV$Subtype_2) == "TCF3-PBX1"] <- "Others"
levels(norm_CoxUV$Subtype_2)[levels(norm_CoxUV$Subtype_2) == "PAX5alt"] <- "Others"
table(norm_CoxUV$Subtype_2)

norm_CoxUV$Subtype_2 <- as.factor(norm_CoxUV$Subtype_2)
norm_CoxUV$Subtype_2 <- relevel(norm_CoxUV$Subtype_2, ref = "HHD")

cox_covar_subtype = coxph(Surv(eventTime, EFSb) ~ Subtype_2, data = norm_CoxUV, x = T)
round(cox.zph(cox_covar_subtype)$table, 2)
summary(cox_covar_subtype)

cox_covar_subtype = coxph(Surv(eventTime, EFSb) ~ predGr + Subtype_2, data = norm_CoxUV, x = T)
round(cox.zph(cox_covar_subtype)$table, 2)
summary(cox_covar_subtype)

cox_covar_subtype = coxph(Surv(eventTime, EFSb) ~ predRisk + Subtype_2, data = norm_CoxUV, x = T)
round(cox.zph(cox_covar_subtype)$table, 2)
summary(cox_covar_subtype)
...
# Two groups
#KM
quartiles <- quantile(norm_surv$predRisk, probs = c(0, 0.7, 1))
intervalos <- cut(norm_surv$predRisk, breaks = quartiles, labels = c("Low-risk", "High-risk"), include.lowest = T)
norm_surv$predGr = as.factor(intervalos)
norm_surv$EFS_Time = as.numeric(as.character(norm_surv$EFS_Time))
norm_surv$EFS_Status = as.numeric(as.character(norm_surv$EFS_Status))
fit <- survfit(Surv(EFS_Time, EFS_Status) ~ predGr, data = norm_surv)
