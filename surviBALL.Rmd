...
We obtain as a result that 94073 genes are minimally expressed in NSG cohort. And now on C2 data:
...
We correct the batch effect from different library preparation kits in C1 cohort:
...
Now we repeat the same for C2 cohort. In this case, the batch correction is done based on the read length for each sample. First group has less than 101 bp read, and the second one 101 bp reads.
...
## C1 + C2 clinical variables

```{r C1 C2 cohort validation, message = F, warning = F}
metaData_clinical <- data.frame(read_excel("Journals/Molecular and Cellular Pediatrics/Revision/Additional File 1_revised.xlsx", sheet = "Additional File 1.a", skip = 1), row.names = 1)

print(identical(rownames(metaData_clinical), colnames(norm)))

norm_clin = cbind(metaData_clinical, t(norm))

#Predictions
#surviBALL = readRDS("C:/Users/834733/Documents/EHU/Lab/ALL/Survival/Paper_bioinfo/surviBALL.rda")
norm_clin$predRisk = predict(surviBALL, newdata = norm_clin)
#Risk plot
norm_clin <- norm_clin[order(norm_clin$predRisk), ]
norm_clin$Order <- 1:nrow(norm_clin)
ggplot(norm_clin, aes(x = Order, y = predRisk, color = as.character(EFS_5yrs))) +
  geom_point(size = 2) +
  scale_color_manual(values = c("1" = "#EE5C42", "0" = "#00EE76")) +
  labs(x = "Ordered by Risk-score", y = "Risk-score", title = "TARGET surviBALL") +
  theme_minimal()
#KM
quartiles <- quantile(norm_clin$predRisk, probs = c(0, 0.7, 0.9, 1))
intervalos <- cut(norm_clin$predRisk, breaks = quartiles, labels = c("Low-risk", "Intermediate-risk", "High-risk"), include.lowest = T)
norm_clin$predGr = as.factor(intervalos)
norm_clin$eventTime_5yrs = as.numeric(as.character(norm_clin$eventTime_5yrs))
norm_clin$EFS_5yrs = as.numeric(as.character(norm_clin$EFS_5yrs))
fit <- survfit(Surv(eventTime_5yrs, EFS_5yrs) ~ predGr, data = norm_clin)
#summary(fit)
(log_rank_test <- survdiff(Surv(eventTime_5yrs, EFS_5yrs) ~ predGr, data = norm_clin))
KM = ggsurvplot(fit,
                pval = T,
                conf.int = F,
                risk.table = T,
...
print(KM)
# Comparación entre grupos
(pairwise_results <- pairwise_survdiff(Surv(eventTime_5yrs, EFS_5yrs) ~ predGr, data = norm_clin, p.adjust.method = "BH"))

norm_clin$Subtype2 <- as.factor(as.character(norm_clin$subtype_def))
norm_clin$Subtype2 <- relevel(norm_clin$Subtype2, ref = "HHD")
table(norm_clin$Subtype2)
levels(norm_clin$Subtype2)[levels(norm_clin$Subtype2) == "PAX5_P80R"] <- "Others"
levels(norm_clin$Subtype2)[levels(norm_clin$Subtype2) == "IKZF1_N159Y"] <- "Others"
levels(norm_clin$Subtype2)[levels(norm_clin$Subtype2) == "ZNF384r-like"] <- "Others"
levels(norm_clin$Subtype2)[levels(norm_clin$Subtype2) == "ETV6::RUNX1-like"] <- "Others"
levels(norm_clin$Subtype2)[levels(norm_clin$Subtype2) == "PAX5alt"] <- "Others"
levels(norm_clin$Subtype2)[levels(norm_clin$Subtype2) == "TCF3::PBX1"] <- "Others"

TARGET_predVal = coxph(Surv(eventTime_5yrs, EFS_5yrs) ~ Subtype2, data = norm_clin, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table["GLOBAL",], 2)
ggforest(TARGET_predVal)

TARGET_predVal = coxph(Surv(eventTime_5yrs, EFS_5yrs) ~ predRisk + Subtype2, data = norm_clin, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table["GLOBAL",], 2)
ggforest(TARGET_predVal)

TARGET_predVal = coxph(Surv(eventTime_5yrs, EFS_5yrs) ~ predGr, data = norm_clin, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table["GLOBAL",], 2)
ggforest(TARGET_predVal)

TARGET_predVal = coxph(Surv(eventTime_5yrs, EFS_5yrs) ~ sex, data = norm_clin, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table["GLOBAL",], 2)
ggforest(TARGET_predVal)


# Boxplot con expresion por subtipos
norm_surv_boxplots = cbind(metaData_TARGET, t(norm_T))
norm_surv_boxplots$Subtype3 <- as.factor(as.character(norm_surv_boxplots$subtype_def))
norm_surv_boxplots$Subtype3 <- relevel(norm_surv_boxplots$Subtype3, ref = "HHD")

levels(norm_surv_boxplots$Subtype3)[levels(norm_surv_boxplots$Subtype3) == "PAX5_P80R"] <- "Others"
levels(norm_surv_boxplots$Subtype3)[levels(norm_surv_boxplots$Subtype3) == "ZNF384r-like"] <- "Others"
levels(norm_surv_boxplots$Subtype3)[levels(norm_surv_boxplots$Subtype3) == "DUX4r"] <- "Others"

# global difs
kruskal.test(lnckb.8576 ~ Subtype2, data = norm_clin)
pw12_8576 <- pairwise.wilcox.test(norm_clin$lnckb.8576,
                     norm_clin$Subtype2,
                     p.adjust.method = "bonferroni")$p.value
ggplot(norm_clin, aes(x = Subtype2, y = lnckb.8576)) +
  geom_boxplot(aes(fill=as.factor(Subtype2))) +
  theme_bw() +
  ylab("lnckb.8576 expression") +
  xlab("Subtypes") +
  ggtitle("Boxplot with lncRNA expression per subtype") +
  scale_fill_manual(values = c("#9857A4", "#53A054", "#C26060", "#E0A840", "#4C88C2", "#D970C2", "#69C2B0", "#737373", "#F2855E"), name = "Subtypes")

kruskal.test(lnckb.32044 ~ Subtype2, data = norm_clin)
pw12_32044 <- pairwise.wilcox.test(norm_clin$lnckb.32044,
                     norm_clin$Subtype2,
                     p.adjust.method = "bonferroni")$p.value
ggplot(norm_clin, aes(x = Subtype2, y = lnckb.32044)) +
  geom_boxplot(aes(fill=as.factor(Subtype2))) +
  theme_bw() +
  ylab("lnckb.32044 expression") +
  xlab("Subtypes") +
  ggtitle("Boxplot with lncRNA expression per subtype") +
  scale_fill_manual(values = c("#9857A4", "#53A054", "#C26060", "#E0A840", "#4C88C2", "#D970C2", "#69C2B0", "#737373", "#F2855E"), name = "Subtypes")

kruskal.test(lnckb.61426 ~ Subtype2, data = norm_clin)
pw12_61426 <- pairwise.wilcox.test(norm_clin$lnckb.61426,
                     norm_clin$Subtype2,
                     p.adjust.method = "bonferroni")$p.value

kruskal.test(lnckb.66409 ~ Subtype2, data = norm_clin)
pw12_66409 <- pairwise.wilcox.test(norm_clin$lnckb.66409,
                     norm_clin$Subtype2,
                     p.adjust.method = "bonferroni")$p.value

kruskal.test(lnckb.83321 ~ Subtype2, data = norm_clin)
pw12_83321 <- pairwise.wilcox.test(norm_clin$lnckb.83321,
                     norm_clin$Subtype2,
                     p.adjust.method = "bonferroni")$p.value



ggplot(norm_clin, aes(x = Subtype2, y = lnckb.66409)) +
  geom_boxplot(aes(fill=as.factor(Subtype2))) +
  theme_bw() +
  ylab("lnckb.66409 expression") +
  xlab("Subtypes") +
  ggtitle("Boxplot with lncRNA expression per subtype") +
  scale_fill_manual(values = c("#9857A4", "#53A054", "#C26060", "#E0A840", "#4C88C2", "#D970C2", "#69C2B0", "#737373", "#F2855E"), name = "Subtypes")

ggplot(norm_clin, aes(x = Subtype2, y = lnckb.61426)) +
  geom_boxplot(aes(fill=as.factor(Subtype2))) +
  theme_bw() +
  ylab("lnckb.61426 expression") +
  xlab("Subtypes") +
  ggtitle("Boxplot with lncRNA expression per subtype") +
  scale_fill_manual(values = c("#9857A4", "#53A054", "#C26060", "#E0A840", "#4C88C2", "#D970C2", "#69C2B0", "#737373", "#F2855E"), name = "Subtypes")
...
ggplot(norm_clin, aes(x = Subtype2, y = lnckb.8576)) +
  geom_boxplot(aes(fill=as.factor(Subtype2))) +
  theme_bw() +
  ylab("lnckb.83321 expression") +
  xlab("Subtypes") +
  ggtitle("Boxplot with lncRNA expression per subtype") +
  scale_fill_manual(values = c("#9857A4", "#53A054", "#C26060", "#E0A840", "#4C88C2", "#D970C2", "#69C2B0", "#737373", "#F2855E"), name = "Subtypes")

dunn_85_12 <- norm_clin %>% dunn_test(lnckb.8576 ~ Subtype2, p.adjust.method = "bonferroni")
dunn_32_12 <- norm_clin %>% dunn_test(lnckb.32044 ~ Subtype2, p.adjust.method = "bonferroni")
dunn_61_12 <- norm_clin %>% dunn_test(lnckb.61426 ~ Subtype2, p.adjust.method = "bonferroni")
dunn_66_12 <- norm_clin %>% dunn_test(lnckb.66409 ~ Subtype2, p.adjust.method = "bonferroni")
dunn_83_12 <- norm_clin %>% dunn_test(lnckb.83321 ~ Subtype2, p.adjust.method = "bonferroni")
```
...
#metaData_TARGET <- data.frame(read_excel("Journals/Translational Research/Appendix A.xlsx", sheet = "Appendix A.2", skip = 1), row.names = 7)

#revision
metaData_TARGET <- data.frame(read_excel("Journals/Molecular and Cellular Pediatrics/Revision/Additional File 1_revised.xlsx", sheet = "Additional File 1.b", skip = 1), row.names = 8)
...
order = sort(colnames(countData_TARGET))
metaData_TARGET <- metaData_TARGET[order, ]
countData_TARGET <- countData_TARGET[, order]
...
norm_surv_original <- norm_surv

norm_surv = cbind(metaData_TARGET, t(scaled_T))

#Predictions
#surviBALL = readRDS("C:/Users/834733/Documents/EHU/Lab/ALL/Survival/Paper_bioinfo/surviBALL.rda")
norm_surv$predRisk = predict(surviBALL, newdata = norm_surv)
norm_surv$EFS_Status <- as.numeric(norm_surv$EFS_Status)

norm_surv$Subtype2 <- as.factor(as.character(norm_surv$Subtype2))
levels(norm_surv$Subtype2)[levels(norm_surv$Subtype2) == "PAX5_P80R"] <- "Others"

norm_surv$Subtype <- as.factor(as.character(norm_surv$Subtype))
norm_surv$Subtype <- relevel(norm_surv$Subtype, ref = "Hyperdiploidy")

levels(norm_surv$Subtype)[levels(norm_surv$Subtype) == "PAX5_P80R"] <- "Others"
levels(norm_surv$Subtype)[levels(norm_surv$Subtype) == "DUX4-rearranged"] <- "Others"
levels(norm_surv$Subtype)[levels(norm_surv$Subtype) == "KMT2A_or_like"] <- "Others"
levels(norm_surv$Subtype)[levels(norm_surv$Subtype) == "MEF2D_rearrangement"] <- "Others"

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ Age, data = norm_surv, x = T)
round(cox.zph(TARGET_predVal)$table, 2)
summary(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ Sex, data = norm_surv, x = T)
round(cox.zph(TARGET_predVal)$table, 2)
summary(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ Subtype2, data = norm_surv, x = T)
round(cox.zph(TARGET_predVal)$table, 2)
summary(TARGET_predVal)
ggforest(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ predGr, data = norm_surv, x = T)
round(cox.zph(TARGET_predVal)$table, 2)
summary(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ predRisk, data = norm_surv, x = T)
round(cox.zph(TARGET_predVal)$table, 2)
summary(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ MRD_.29_Status_0.01., data = norm_surv, x = T)
round(cox.zph(TARGET_predVal)$table, 2)
summary(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ MRD_.29, data = norm_surv, x = T)
round(cox.zph(TARGET_predVal)$table, 2)
summary(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ predRisk + Subtype, data = norm_surv, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table, 2)
ggforest(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ predGr + Subtype, data = norm_surv, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table, 2)
ggforest(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ MRD_.29_Status_0.01. + Subtype, data = norm_surv, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table, 2)
ggforest(TARGET_predVal)

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ predGr + Subtype + MRD_.29_Status_0.01., data = norm_surv, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table, 2)
ggforest(TARGET_predVal)
...
ggplot(norm_surv, aes(x = Order, y = predRisk, color = as.character(EFS_Status))) +
  geom_point(size = 2) +
  scale_color_manual(values = c("1" = "#EE5C42", "0" = "#00EE76")) +
  labs(x = "Ordered by Risk-score", y = "Risk-score", title = "TARGET surviBALL") +
  theme_minimal()
...
norm_surv$EFS_Status = as.numeric(as.character(norm_surv$EFS_Status))
fit <- survfit(Surv(EFS_Time, EFS_Status) ~ predGr, data = norm_surv)
summary(fit)
(log_rank_test <- survdiff(Surv(EFS_Time, EFS_Status) ~ predGr, data = norm_surv))
KM = ggsurvplot(fit,
                pval = T,
                conf.int = F,
                risk.table = T,
...
print(KM)
# Comparación entre grupos
(pairwise_results <- pairwise_survdiff(Surv(EFS_Time, EFS_Status) ~ predGr, data = norm_surv, p.adjust.method = "BH"))
```
...
sig_terms <- c("leukocyte activation involved in immune response", "leukocyte degranulation", "response to chemical", "FOXO-mediated transcription")
...
ggplot(Bubbles, aes(x = order, y = `-log(q-value)`)) + 
  ...
  labs(title = "ORA result as bubbles plot",
       ...
       size = "Genes in pathway") +
  geom_label_repel(
    data = subset(Bubbles, term_name %in% sig_terms & `-log(q-value)` > 1.3),
    aes(label = term_name),
    size = 3,
    nudge_y = 1,   # desplaza el texto un poco hacia arriba
    arrow = arrow(length = unit(0.015, "npc")),  # dibuja flecha
    box.padding = 0.5
  )
```

## TARGET cohort validation revision

```{r TARGET cohort validation rev, message = F, warning = F}
metaData_TARGET <- data.frame(read_excel("Journals/Molecular and Cellular Pediatrics/Revision/Additional File 1_revised.xlsx", sheet = "Additional File 1.b", skip = 1), row.names = 8)
...
norm_surv$Subtype <- as.factor(as.character(norm_surv$Subtype))
norm_surv$Subtype <- relevel(norm_surv$Subtype, ref = "HHD")

levels(norm_surv$Subtype)[levels(norm_surv$Subtype) == "PAX5_P80R"] <- "Others"
levels(norm_surv$Subtype)[levels(norm_surv$Subtype) == "DUX4r"] <- "Others"
levels(norm_surv$Subtype)[levels(norm_surv$Subtype) == "ZNF384r-like"] <- "Others"

TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ Subtype, data = norm_surv, x = T)
summary(TARGET_predVal)
ggforest(TARGET_predVal)
...
TARGET_predVal = coxph(Surv(EFS_Time, EFS_Status) ~ predGr + Subtype + MRD_.29_Status_0.01., data = norm_surv, x = T)
summary(TARGET_predVal)
round(cox.zph(TARGET_predVal)$table["GLOBAL",], 3)
ggforest(TARGET_predVal)
...
dunn_85 <- norm_surv_boxplots %>% dunn_test(lnckb.8576 ~ Subtype3, p.adjust.method = "bonferroni")
dunn_32 <- norm_surv_boxplots %>% dunn_test(lnckb.32044 ~ Subtype3, p.adjust.method = "bonferroni")
dunn_61 <- norm_surv_boxplots %>% dunn_test(lnckb.61426 ~ Subtype3, p.adjust.method = "bonferroni")
dunn_66 <- norm_surv_boxplots %>% dunn_test(lnckb.66409 ~ Subtype3, p.adjust.method = "bonferroni")
dunn_83 <- norm_surv_boxplots %>% dunn_test(lnckb.83321 ~ Subtype3, p.adjust.method = "bonferroni")

ggplot(norm_surv_boxplots, aes(x = Subtype3, y = lnckb.32044, color = as.factor(Subtype3))) +
  geom_jitter(width = 0.2, alpha = 1, size = 2) +
  stat_summary(fun = median, geom = "crossbar", 
               width = 0.5, fatten = 2, color = "black") +
  stat_summary(fun.data = function(y) {
    r <- quantile(y, probs = c(0.25, 0.75))
    data.frame(y = r, ymin = r[1], ymax = r[2])
  }, geom = "errorbar", width = 0.3, color = "black") +
  theme_bw() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  ylab("lnckb.32044 expression") +
  xlab("Subtypes") +
  ggtitle("lnckb.32044 expression per subtype") +
  scale_color_manual(values = c("#9857A4", "#53A054", "#C26060", "#E0A840", "#4C88C2", "#D970C2", "#69C2B0", "#737373", "#F2855E", "#5A9E91", "#B589D6", "#3D6D94", "#C4C45A"), name = "Subtypes")

ggplot(norm_surv_boxplots, aes(x = Subtype3, y = lnckb.8576, color = as.factor(Subtype3))) +
  geom_jitter(width = 0.2, alpha = 1, size = 2) +
  stat_summary(fun = median, geom = "crossbar", 
               width = 0.5, fatten = 2, color = "black") +
  stat_summary(fun.data = function(y) {
  ...
  ylab("lnckb.8576 expression") +
  xlab("Subtypes") +
  ggtitle("lnckb.8576 expression per subtype") +
  scale_color_manual(values = c("#9857A4", "#53A054", "#C26060", "#E0A840", "#4C88C2", "#D970C2", "#69C2B0", "#737373", "#F2855E", "#5A9E91", "#B589D6", "#3D6D94", "#C4C45A"), name = "Subtypes")
